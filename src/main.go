...
// server模式  go run main.go server.go client.go handler.go  -server true -P binary -framed false -buffered false -addr localhost:9090 -secure false
// client模式  go run main.go server.go client.go handler.go  -server false -P binary -framed false -buffered false -addr localhost:9090 -secure false
// goland运行需要配置 run kind package
func main() {
	flag.Usage = Usage
